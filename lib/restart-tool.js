 ...
 */
export function isPluginTreeChanged(harnessRoot, lkgDir = join(homedir(), '.dsh/.supervisor/lkg')) {
    void harnessRoot;
    try {
        ...
        if (existsSync(livePlugins)) {
            for (const name of readdirSync(livePlugins)) {
                ...
                if (!existsSync(libDir))
                    continue;
                if (newestFileMtime(libDir) > baseline)
                    return true;
            }
        }
        return false;
...
function newestFileMtime(dir) {
    ...
    for (const entry of readdirSync(dir)) {
        ...
        const st = statSync(p);
        if (st.isDirectory())
            newest = Math.max(newest, newestFileMtime(p));
        else
