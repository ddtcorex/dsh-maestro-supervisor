...
async function autoFixKnownPatterns(err: string, exec: (c: string, o?: any) => string, readFile: (p: string) => string, writeFile: (p: string, c: string) => void): Promise<void> {
  ...
  if (lower.includes('allowbuilds') || lower.includes('allow_builds')) {
    try { exec('pnpm --dir /home/kai/Work/htdocs/maestro-harness/packages/dsh-maestro-supervisor verify --silent 2>&1 | head -5', { timeout: 15000 }) } catch {}
  ...
  if (err.includes('ERR_MODULE_NOT_FOUND') || err.includes('Cannot find module')) {
    ...
    for (const pkg of candidates) {
      try {
        ...
        readFile(p)
        try { exec(`pnpm --dir /home/kai/Work/htdocs/maestro-harness/packages/${pkg} verify --silent 2>&1 | head -5`, { timeout: 15000 }) } catch {}
      } catch {}
    }
    return
  }
...
async function resolveLLMConfig(): Promise<LLMConfig> {
  ...
  try {
    ...
    if (typeof m === 'string' && !process.env.AI_MODEL && !process.env.DEEPSEEK_MODEL) model = m
    else if (m?.model && typeof m.model === 'string' && !process.env.AI_MODEL) model = m.model
  } catch {}
