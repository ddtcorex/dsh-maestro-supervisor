...
interface ScanOptions { withinMs?: number }

async function decodeOk(file: string): Promise<boolean> {
  try {
 ...
 */
export async function scanSessions(dshHome: string, opts: ScanOptions = {}): Promise<{ scanned: number; torn: string[] }> {
    ...
    for (const proj of readdirSync(sessionsRoot)) {
      ...
      for (const sess of readdirSync(projDir)) {
        ...
        for (const f of readdirSync(sessDir)) {
          ...
          if (!name.endsWith('.zstd') && !name.endsWith('.jsonl')) continue
          const fp = join(sessDir, f)
          let mtime = 0
          try { mtime = statSync(fp).mtimeMs } catch { continue }
          if (now - mtime > withinMs) continue
          files.push(fp)
        }
      }
    }
  ...
  for (const f of files) {
    if (!(await decodeOk(f))) torn.push(f)
  }
