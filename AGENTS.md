...
- `src/host/plugin.ts` — in-tree Cordis host plugin: `inject: ['sessions','agents','connection']`, `apply()` (never throws), `runAutoResume()` (merges interrupted + dangling, 5m window), `resumeInterrupted()` (agents.get → agents.resume + recover provider/model + followup continue) — a resume blocked by another owner's write handle (the reconnecting browser re-opens the interrupted session while the boot scan runs) is retried with bounded backoff, and if the host still owns it the recovery prompt is delivered through the `sessionController.prompt` API the UI uses (the session the browser re-opened cannot be taken by `agents.resume` at all); both delivery paths gate on `waitForCriticalTools()` (bash registered) before sending, because a resume delivered earlier freezes a request header without bash, and both then run the C1/C2 tool-view probe, `createResumeRpcHandler()` (`scan`/`resume` endpoints)
